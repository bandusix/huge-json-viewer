...
- 🔒 **文件永不离开你的设备**——不上传文件、无服务器 · 仅发送匿名、可随时关闭的使用统计 · 应用极小
- 🆓 **免费且开源**（MIT）
...
## BigJSON vs. Dadroit vs. 文本编辑器

一份基于事实、逐项对比的比较。**BigJSON 是免费、开源的 [Dadroit](https://dadroit.com) 替代方案：**下表所列的一切——大文件、搜索、CSV/XML 导出、多文件合并、商业使用——全部**免费**提供，而 Dadroit 则将它们锁在 **$98–$198/yr** 的付费档位之后。

| | **BigJSON** | **Dadroit** | 文本编辑器（VS Code 等） |
| --- | --- | --- | --- |
| **价格** | **免费且开源（MIT）** | 免费 *非商业用途，≤ 50 MB* · **$98/yr**（≤ 2 GB）· **$198/yr**（≤ 1 TB） | 免费 / 付费 |
| **商业使用** | ✅ **免费** | 💲 付费（$98/yr 起）——免费档位仅限非商业用途 | ✅ |
| **开源** | ✅ MIT，可审计 | ❌ 闭源 | 不一 |
| **免费上限** | **4 GB** | 50 MB，超出则需付费 | — |
| **最大文件大小** | 4 GB / 文件 | **1 TB**（付费档位） | 约几百 MB 便会卡死 |
| **打开 2–3 GB JSON** | ✅ 约 3 秒 | ✅（付费档位） | ❌ 崩溃 / 卡死 |
| **原始打开吞吐量** | 快（~1 GB/s，受 CPU 限制） | 非常快（厂商宣称 ~2 GB/s） | 慢 |
| **打开 3 GB 文件的内存占用** | ~1.5–2×（索引；映射的文件仍是可回收的页缓存） | ~1×（厂商宣称） | 经常内存溢出 |
| **可折叠的树形查看器** | ✅ | ✅ | ❌（纯文本） |
| **搜索键与值** | ✅ | ✅ | 有限 |
| **RegEx 搜索** | ✅ | ✅ | ✅ |
| **转换 → CSV / XML** | ✅ **免费** | ✅ | ❌ |
| **将子树导出 / 复制为 JSON** | ✅ | 节点级导出 | 手动 |
| **复制键 / 值 / 路径（jq 风格）** | ✅ | 值 + 导出 | 手动复制粘贴 |
| **合并多个文件** | ✅ **免费** | 💲 付费档位 | ❌ |
| **NDJSON / JSON Lines** | ✅ 自动识别 | ✅ | ❌ |
| **文件变动时自动刷新** | ❌ | ✅ | 部分支持 |
| **编辑 JSON** | ❌ 仅查看 | ❌ 仅查看 | ✅ |
| **平台** | macOS（通用）· Windows | Windows · macOS · **Linux** | 全部 |
| **界面语言** | **20 种（支持 RTL）** | 少 | 多 |
| **文件永不离开你的设备** | ✅ | ✅ | ✅ |
| **安装体积** | ~2–5 MB | 数十 MB | — |

**简而言之：**对于几 GB 以内的 JSON，BigJSON 能做到 Dadroit 付费档位所做的一切——打开、正则搜索、转换为 CSV/XML、合并多个文件、提取子树——而且**免费、开源，没有商业使用或功能付费墙**。Dadroit 的免费档位止步于 **50 MB** 且禁止商业使用；解锁 2 GB / 商业用途需 **$98/yr**，1 TB 加全部功能则需 **$198/yr**。

**Dadroit 仍然领先之处（如实而言）：** 超过 4 GB 的文件（最高可达 **1 TB**）、原生 **Linux** 构建、文件在磁盘上变动时的**自动刷新**，以及在其自家基准测试中更高的原始吞吐量。如果你经常打开 100 GB–1 TB 的文件或需要 Linux，Dadroit 值得它的授权费。而对于几 GB 以内的一切，BigJSON 是**免费、无限制**的选择。

<sub>Dadroit 的档位与定价依据 [dadroit.com](https://dadroit.com/buy-licence/)（可能变动）。速度/内存数据均为各项目自身的宣称——请用你自己的文件做基准测试来对比。</sub>
