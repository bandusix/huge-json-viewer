...
> Ouvrez et recherchez dans des **fichiers JSON volumineux (2–3 Go et plus)** sur **macOS** en quelques secondes — une **alternative gratuite et open source à Dadroit**. Quand votre éditeur de texte ou votre navigateur plante sur un gros fichier JSON, cette visionneuse JSON volumineux l'ouvre instantanément.
...
### [⬇️ Télécharger le dernier DMG](https://github.com/bandusix/huge-json-viewer/releases/latest)
...
- 🖱️ Glisser-déposer, ⌘O pour ouvrir, ⌘F pour rechercher, navigation entièrement au clavier
- 🌍 **Interface en 20 langues**, compatible droite-à-gauche (arabe, ourdou, pendjabi)
- 🖥️ **Universelle** — Apple Silicon (M1–M4) *et* Intel
...
2. Faites glisser **Huge JSON Viewer** dans Applications.
...
Configuration requise : macOS 11 (Big Sur) ou plus récent. Prend en charge les fichiers JSON jusqu'à 4 Go.
...
Construite avec **Tauri v2** (backend Rust + frontend web), empaquetée en un `.dmg` d'environ 2 Mo.

## Huge JSON Viewer face aux autres outils pour gros JSON

| | Huge JSON Viewer | Dadroit | Éditeurs de texte (VS Code, etc.) |
| --- | --- | --- | --- |
...
| Natif macOS (Apple Silicon + Intel) | ✅ universel | ✅ | ✅ |
...
- **RAM :** l'index occupe ~23 octets/nœud, donc un fichier de 2–3 Go nécessite environ **1,5–2× la taille du fichier** en RAM (le fichier memory-mappé lui-même est un cache de pages du système, évictable). Un Mac de 16 Go gère confortablement des fichiers de 2–3 Go.
...
<sub>Mots-clés : ouvrir un gros fichier JSON, visionneuse JSON volumineux, ouvrir un JSON de 2 Go/3 Go, fichier JSON trop gros, visionneuse JSON pour Mac, alternative gratuite à Dadroit, visionneuse JSON open source, rechercher dans un gros JSON, JSON de plusieurs gigaoctets, visionneuse JSON en streaming.</sub>
