...
- 🔒 **ملفاتك لا تغادر جهازك أبدًا** — بلا رفع للملفات، بلا خادم، على الإطلاق · فقط إحصاءات استخدام مجهولة الهوية يمكن إيقافها · تطبيق صغير الحجم
...
## BigJSON مقابل Dadroit مقابل محررات النصوص

مقارنة واقعية، ميزةً بميزة. **BigJSON هو البديل المجاني مفتوح المصدر لـ [Dadroit](https://dadroit.com):** كل ما هو أدناه — الملفات الكبيرة، والبحث، والتصدير إلى CSV/XML، ودمج عدة ملفات، والاستخدام التجاري — متضمَّن **بلا أي تكلفة**، بينما يحجبه Dadroit خلف فئات مدفوعة بسعر **98–198 دولارًا سنويًا**.

| | **BigJSON** | **Dadroit** | محررات النصوص (VS Code وغيرها) |
| --- | --- | --- | --- |
| **السعر** | **مجاني ومفتوح المصدر (MIT)** | مجاني *لغير التجاري، ≤ 50 MB* · **$98/yr** (≤ 2 GB) · **$198/yr** (≤ 1 TB) | مجاني / مدفوع |
| **الاستخدام التجاري** | ✅ **مجاني** | 💲 مدفوع ($98/yr فما فوق) — الفئة المجانية لغير التجاري | ✅ |
| **مفتوح المصدر** | ✅ MIT، قابل للتدقيق | ❌ مغلق المصدر | متفاوت |
| **مجاني حتى** | **4 GB** | 50 MB، ثم مدفوع | — |
| **أقصى حجم للملف** | 4 GB / ملف | **1 TB** (فئة مدفوعة) | ~بضع مئات من الميجابايت قبل أن يختنق |
| **فتح JSON بحجم 2–3 GB** | ✅ ~3 ثوانٍ | ✅ (فئة مدفوعة) | ❌ ينهار / يتجمد |
| **معدل الفتح الخام** | سريع (~1 GB/s، مقيّد بالمعالج) | سريع جدًا (ادعاء المورّد ~2 GB/s) | بطيء |
| **ذاكرة RAM لملف 3 GB** | ~1.5–2× (الفهرس؛ والملف المربوط يبقى ذاكرة تخزين صفحات قابلة للاسترجاع) | ~1× (ادعاء المورّد) | غالبًا نفاد الذاكرة |
| **عارض شجري قابل للطي** | ✅ | ✅ | ❌ (نص خام) |
| **البحث في المفاتيح والقيم** | ✅ | ✅ | محدود |
| **بحث RegEx** | ✅ | ✅ | ✅ |
| **تحويل → CSV / XML** | ✅ **مجاني** | ✅ | ❌ |
| **تصدير / نسخ شجرة فرعية بصيغة JSON** | ✅ | تصدير على مستوى العقدة | يدويًا |
| **نسخ المفتاح / القيمة / المسار (بأسلوب jq)** | ✅ | القيمة + التصدير | نسخ ولصق يدوي |
| **دمج عدة ملفات** | ✅ **مجاني** | 💲 فئة مدفوعة | ❌ |
| **NDJSON / JSON Lines** | ✅ يُكتشف تلقائيًا | ✅ | ❌ |
| **تحديث تلقائي عند تغيّر الملف** | ❌ | ✅ | بعضها |
| **تحرير JSON** | ❌ للعرض فقط | ❌ للعرض فقط | ✅ |
| **المنصات** | macOS (شاملة) · Windows | Windows · macOS · **Linux** | الكل |
| **لغات الواجهة** | **20 (تدعم RTL)** | قليلة | كثيرة |
| **ملفاتك لا تغادر جهازك أبدًا** | ✅ | ✅ | ✅ |
| **حجم التثبيت** | ~2–5 MB | عشرات الميجابايت | — |

**باختصار:** بالنسبة إلى JSON حتى بضعة جيجابايت، يفعل BigJSON كل ما تفعله الفئات المدفوعة من Dadroit — الفتح، والبحث بـ regex، والتحويل إلى CSV/XML، ودمج عدة ملفات، واستخراج الأشجار الفرعية — **مجانًا، ومفتوح المصدر، ودون أي جدار دفع للاستخدام التجاري أو الميزات**. تتوقّف الفئة المجانية من Dadroit عند **50 MB** وتمنع الاستخدام التجاري؛ ويكلّف فتح 2 GB / الاستخدام التجاري **$98/yr**، بينما يكلّف 1 TB مع كامل مجموعة الميزات **$198/yr**.

**حيث لا يزال Dadroit متفوّقًا (بأمانة):** الملفات التي تتجاوز 4 GB (حتى **1 TB**)، ونسخ **Linux** الأصلية، و**التحديث التلقائي** عند تغيّر الملف على القرص، ومعدل خام أعلى في اختباراته الخاصة. إن كنت تفتح بانتظام ملفات بحجم 100 GB–1 TB أو تحتاج إلى Linux، فإن Dadroit يستحق ترخيصه. أما لكل ما دون بضعة جيجابايت، فإن BigJSON هو الخيار **المجاني وغير المحدود**.

<sub>فئات Dadroit وأسعاره وفق [dadroit.com](https://dadroit.com/buy-licence/) (قابلة للتغيير). أرقام السرعة/الذاكرة هي ادعاءات كل مشروع عن نفسه — اختبر ملفاتك الخاصة للمقارنة.</sub>
